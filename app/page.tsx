    ...
    <main className="shell">
      ...
      <div className="workspace-grid">
        ...
        <div className="right-column">
          <section className="panel ask-panel" aria-labelledby="ask-heading"><div className="panel-heading"><div><p className="eyebrow">Ask quietly</p><h2 id="ask-heading">What do you need?</h2></div><span className="keyboard-hint">Enter</span></div><form onSubmit={ask}><textarea value={question} onChange={(event) => setQuestion(event.target.value)} disabled={!meetingId || asking} placeholder={meetingId ? "Who owns the follow-up?" : "Start a meeting to ask questions..."} rows={3} /><div className="ask-footer"><span>Answers use the server-side transcript.</span><button className="ask-button" type="submit" disabled={!meetingId || !question.trim() || asking}>{asking ? "Thinking..." : "Ask"} <span>-&gt;</span></button></div></form></section>
